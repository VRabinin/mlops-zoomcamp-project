...
class TestCRMDataIngestion:
    ...
    def test_load_data_with_sample_dataframe(self):
        ...
        try:
            ...
            with patch.object(
                ingestion.storage, "load_dataframe", return_value=sample_data
            ):
    ...
    def test_save_processed_data(self):
        ...
        with patch.object(
            ingestion.storage, "save_dataframe", return_value=expected_path
        ) as mock_save:
    ...
    def test_calculate_quality_score(self):
        ...
        # Perfect data
        perfect_df = pd.DataFrame({"col1": [1, 2, 3], "col2": ["a", "b", "c"]})
        score = ingestion._calculate_quality_score(perfect_df, [])
        assert score == 1.0

        # Data with missing values
        missing_df = pd.DataFrame({"col1": [1, None, 3], "col2": ["a", "b", None]})
        score_missing = ingestion._calculate_quality_score(missing_df, [])
        assert score_missing < 1.0

        # Data with issues
        score_issues = ingestion._calculate_quality_score(
            perfect_df, ["issue1", "issue2"]
        )
