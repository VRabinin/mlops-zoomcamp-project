...
class CRMDataIngestion:
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numerical_cols:
            if df_cleaned[col].isnull().any():
                median_val = df_cleaned[df_cleaned["deal_stage"].isin(["Won", "Lost"])][
                    col
                ].median()
    ...
    def save_processed_data(
        self, df: pd.DataFrame, filename: str = "crm_data_processed_XXXX-XX.csv"
    ) -> str:
    ...
    def run_ingestion(self) -> Tuple[bool, pd.DataFrame, Dict[str, Any]]:
        ...
        """
        metadata = {"timestamp": pd.Timestamp.now(), "steps_completed": []}
        ...
        try:
            ...
            saved_path = self.save_processed_data(
                df_cleaned, filename=f"crm_data_processed_{self.snapshot_month}.csv"
            )
            ...
            self.logger.info(
                f"Quality score: {metadata['validation']['quality_score']:.2f}"
            )
