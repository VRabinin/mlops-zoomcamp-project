...
class CRMDataAcquisition:
    ...
    def load_data(self, file_path: Optional[Path] = None) -> pd.DataFrame:
        ...
        if file_path is None:
            ...
            if file_path is None:
                raise FileNotFoundError("No main sales_pipeline.csv file found in raw data directory")
    ...
    def enhance_data(self, df_sales: pd.DataFrame) -> Tuple[list[pd.DataFrame], list[str]]:
        ...
        df_sales['creation_date'] = pd.to_datetime(df_sales['engage_date'], errors='coerce')
        ...
        df_sales['creation_date'] = df_sales['creation_date'] - pd.to_timedelta(df_sales['random_number'], unit='D')
        ...
        for period, group in df_sales.groupby('creation_year_month'):
            if period < self.snapshot_month:
                ...
                self.logger.info(f"Saved {len(df_first_group)} records for period {period} to CSV.")
            else:
                group = self._adjust_dates_and_stage(period, group)
                # Save each group to a separate CSV file
                dataframes.append(group)
                filenames.append(f"sales_pipeline_enhanced_{period}.csv")
                self.logger.info(f"Saved {len(group)} records for period {period} to CSV.")
        if len(dataframes) == 0:
            self.logger.error("No dataframes created during enhancement process. Check the configuration parameter 'first_snapshot_month'.")
            return [], []
        return dataframes, filenames

    def save_enhanced_data(self, dataframe_list: list[pd.DataFrame], filename_list: list[str], suffix: str = "raw") -> str:
