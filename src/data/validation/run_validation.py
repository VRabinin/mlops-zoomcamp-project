...
class DataValidationOrchestrator:
    ...
    def load_processed_data(
        self, filename: str = "crm_data_processed.csv"
    ) -> pd.DataFrame:
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Processed data file not found: {filename}. Error: {str(e)}"
            )
    ...
    def run_comprehensive_validation(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        results["validations"]["schema"] = {"passed": is_valid, "issues": issues}
    ...
    def _run_quality_checks(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        results["missing_values"] = {
            ...
            "passed": missing_percentage.max()
            < 10,  # Fail if any column has >10% missing
        }
        ...
        for col in self.schema.required_columns:
            if col in df.columns:
                ...
                if expected_type == "float64" and actual_type not in [
                    "int64",
                    "float64",
                ]:
        ...
        results["data_types"] = {"issues": type_issues, "passed": len(type_issues) == 0}
    ...
    def _run_business_rules(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "deal_stage" in df.columns:
            invalid_stages = set(df["deal_stage"].unique()) - set(
                self.schema.valid_deal_stages
            )
            results["deal_stages"] = {
    ...
    def generate_validation_report(self, results: Dict[str, Any]) -> str:
        ...
        report.append(
            f"Missing Values: {'✅ PASSED' if missing['passed'] else '❌ FAILED'}"
        )
        ...
        report.append(
            f"Duplicates: {'✅ PASSED' if duplicates['passed'] else '❌ FAILED'}"
        )
        if not duplicates["passed"]:
            report.append(
                f"  - {duplicates['count']} duplicate rows ({duplicates['percentage']:.1f}%)"
            )
        ...
        for rule_name, rule_result in business.items():
            ...
            if not rule_result["passed"]:
                ...
                if "invalid_stages" in rule_result:
                    report.append(
                        f"  - Invalid stages: {rule_result['invalid_stages']}"
                    )
                elif "negative_count" in rule_result:
                    report.append(
                        f"  - Negative values: {rule_result['negative_count']}"
                    )
                elif "out_of_range_count" in rule_result:
                    report.append(
                        f"  - Out of range values: {rule_result['out_of_range_count']}"
                    )
...
def main():
    ...
    try:
        ...
        df = validator.load_processed_data(
            f"crm_data_processed_{config.first_snapshot_month}.csv"
        )
