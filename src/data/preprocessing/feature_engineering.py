...
class CRMFeatureEngineer:
    ...
    def create_features(
        ...
        df_features["is_closed"] = (
            df_features["deal_stage"].isin(["Won", "Lost"]).astype(int)
        )
        ...
        product_performance = (
            df_features[df_features["is_closed"] == 1]
            .groupby("product")
            .agg({"is_won": "mean", "close_value": "mean", "opportunity_id": "count"})
            .round(3)
        )
        ...
        df_features["product_win_rate"] = df_features["product"].map(
            product_performance["product_win_rate"]
        )

        # Date features
        # Convert to datetime, handling errors
        df_features["engage_date"] = pd.to_datetime(
            df_features["engage_date"], errors="coerce"
        )
        ...
        # Convert to datetime, handling errors
        df_features["close_date"] = pd.to_datetime(
            df_features["close_date"], errors="coerce"
        )

        # Extract date components for closed deals
        mask = df_features["close_date"].notna()
        df_features.loc[mask, "close_year"] = df_features.loc[
            mask, "close_date"
        ].dt.year
        df_features.loc[mask, "close_month"] = df_features.loc[
            mask, "close_date"
        ].dt.month
        df_features.loc[mask, "close_quarter"] = df_features.loc[
            mask, "close_date"
        ].dt.quarter
        df_features.loc[mask, "close_day_of_week"] = df_features.loc[
            mask, "close_date"
        ].dt.dayofweek
        ...
        df_features = df_features.merge(df_accounts, on="account", how="left")
        # Check missing values after join
        missing_after_join = df_features["account"].isnull().sum().sum()
        if missing_after_join > 0:
            self.logger.warning(
                f"Missing values after join with accounts: {missing_after_join}"
            )
        ...
        df_features["is_repeat_account"] = (
            df_features["account_frequency"] > 1
        ).astype(int)
        ...
        self.logger.info(
            f"Created {len(df_features.columns) - len(df_features.columns)} new features"
        )
    ...
    def scale_numerical_features(
        ...
        numerical_columns = df_scaled.select_dtypes(
            include=[np.number]
        ).columns.tolist()
    ...
    def prepare_target(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.target_column in df.columns:
            ...
            df_target["target_binary"] = (
                df_target[self.target_column] == "Won"
            ).astype(int)
            ...
            df_target["target_multiclass"] = self.label_encoders[
                "target_encoder"
            ].fit_transform(df_target[self.target_column])
    ...
    def run_feature_engineering(
        ...
        df_features = self.create_features(
            df_sales, df_accounts, df_products, df_sales_teams
        )
...
def main():
    ...
    try:
        df_sales = feature_engineer.storage.load_dataframe(
            "processed", f"crm_data_processed_{config.first_snapshot_month}.csv"
        )
        df_sales_teams = feature_engineer.storage.load_dataframe(
            "raw", "sales_teams.csv"
        )
    ...
    df_processed, feature_columns, metadata = feature_engineer.run_feature_engineering(
        df_sales, df_accounts, df_products, df_sales_teams
    )

    # Save processed data with features using smart storage
    saved_path = feature_engineer.storage.save_dataframe(
        df_processed, "features", f"crm_features_{config.first_snapshot_month}.csv"
    )
