...
def prep_db():
    with psycopg.connect(
        "host=localhost port=5433 dbname=monitoring user=postgres password=postgres",
        autocommit=True,
    ) as conn:
...
def calculate_metrics_postgresql(curr, i):
    ...
    # Value Range
    value_range = result["metrics"][4]["result"]["current"]["share_in_range"]

    curr.execute(
        "insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, quantile, value_range) values (%s, %s, %s, %s, %s, %s)",
        (
