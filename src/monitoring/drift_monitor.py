...
class CRMDriftMonitor:
    ...
    def __init__(self, config: Config = None):
        ...
        self.column_mapping = ColumnMapping(
            ...
            numerical_features=[
                'close_value_log', 'days_since_engage', 'sales_cycle_days',
                'agent_win_rate', 'agent_opportunity_count',
                'product_win_rate', 'product_popularity',
                'revenue', 'employees', 'account_frequency',
                'sales_velocity'
            ],
            categorical_features=[
                'close_value_category_encoded', 'engage_month', 'engage_quarter', 
                'engage_day_of_week', 'is_repeat_account', 'should_close_next_month',
                'is_overdue', 'is_early_stage', 'high_value_deal', 'long_sales_cycle',
                'sales_agent_encoded', 'product_encoded', 'account_encoded',
                'manager_encoded', 'regional_office_encoded', 'sector_encoded'
            ]
        )
    ...
    def load_model(self) -> Any:
        ...
        try:
            model = mlflow.sklearn.load_model("models:/monthly_win_probability_model/latest")
    ...
    def create_monthly_features(self, df: pd.DataFrame, current_date: datetime.datetime = None) -> pd.DataFrame:
    ...
    def create_reference_data(self, snapshot_month: str, sample_size: int = 1000) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if len(df) > sample_size:
                df_reference = df[df['is_won'] == 1].sample(n=sample_size, random_state=42)
                self.logger.info(f"Sampled {sample_size} records from {len(df)} total records")
            else:
            ...
            self.logger.info(f"📈 Win rate in reference: {df_reference['is_won'].mean():.3f}")
    ...
    def generate_current_predictions(self, current_month: str) -> Tuple[bool, str]:
        ...
        """
        try:
            self.logger.info(f"Generating predictions for current month: {current_month}")
            ...
            self.logger.info(f"✅ Current predictions generated and saved: {current_file}")
    ...
    def detect_drift(self, reference_month: str, current_month: str) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        try:
            ...
            monitoring_features = [col for col in self.feature_columns if col in common_columns]
            
            # Create Evidently reports
            drift_report = self._create_drift_report(df_reference, df_current, monitoring_features)
            model_performance_report = self._create_model_performance_report(df_reference, df_current)
            
            # Extract key metrics
            drift_results = self._extract_drift_metrics(drift_report, model_performance_report)
            
            # Save reports
            self._save_drift_reports(drift_report, model_performance_report, reference_month, current_month)
    ...
    def _create_drift_report(self, reference_data: pd.DataFrame, current_data: pd.DataFrame, 
    ...
    def _create_model_performance_report(self, reference_data: pd.DataFrame, 
    ...
    def _extract_drift_metrics(self, drift_report: Report, performance_report: Report) -> Dict[str, Any]:
        ...
        for metric in drift_dict.get('metrics', []):
            ...
            elif metric['metric'] == 'ColumnDriftMetric' and metric.get('result', {}).get('column_name') == 'prediction':
                prediction_drift = metric['result']['drift_score']
            elif metric['metric'] == 'DatasetMissingValuesMetric':
                missing_values_share = metric['result']['current']['share_of_missing_values']
        ...
        return {
            ...
            'f1_score': f1_score,
            'roc_auc': roc_auc,
            'drift_detected': dataset_drift or (prediction_drift and prediction_drift > 0.1),
            'alert_level': self._determine_alert_level(dataset_drift, prediction_drift, num_drifted_columns)
        }
    
    def _determine_alert_level(self, dataset_drift: bool, prediction_drift: float, 
                              num_drifted_columns: int) -> str:
    ...
    def _save_drift_reports(self, drift_report: Report, performance_report: Report,
                           reference_month: str, current_month: str):
        ...
        performance_html = f"performance_report_{reference_month}_{current_month}_{timestamp}.html"
    ...
    def run_data_quality_tests(self, current_month: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            summary = {
                ...
                'failed_tests': sum(1 for test in test_results['tests'] if test['status'] == 'FAIL'),
                'all_tests_passed': all_passed,
                'test_details': test_results['tests']
            }
            
            self.logger.info(f"✅ Data quality tests completed: {summary['passed_tests']}/{summary['total_tests']} passed")
