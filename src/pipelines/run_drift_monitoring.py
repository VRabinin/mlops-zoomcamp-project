...
def generate_current_predictions_task(
    config: Config, current_month: str
) -> Tuple[bool, str]:
    """
...
def run_data_quality_tests_task(
    config: Config, current_month: str
...
def detect_model_drift_task(
    config: Config, reference_month: str, current_month: str
) -> Tuple[bool, Dict[str, Any]]:
    """
    ...
    try:
        ...
        # Detect drift
        success, drift_results = drift_monitor.detect_drift(
        ...
        if success:
            ...
            if drift_detected:
                ...
                if drift_results.get("prediction_drift"):
                    logger.warning(
                        f"🎯 Prediction drift score: {drift_results['prediction_drift']:.3f}"
                    )
                logger.warning(
                    f"📈 Number of drifted columns: {drift_results.get('num_drifted_columns', 0)}"
                )
            else:
...
def save_monitoring_results_task(
    ...
    try:
        ...
        monitoring_summary = {
            ...
            and quality_results.get("all_tests_passed", False)
...
def send_drift_alerts_task(
    config: Config, current_month: str, drift_results: Dict[str, Any]
) -> Tuple[bool, str]:
    """
...
def drift_monitoring_flow(
    current_month: str, reference_month: str = ""
) -> Dict[str, Any]:
    """
    ...
    pred_success, pred_message = generate_current_predictions_task(
    ...
    # Step 3: Detect drift
    drift_success, drift_results = detect_model_drift_task(
        config, reference_month, current_month
    )
    ...
    alert_success, alert_message = send_drift_alerts_task(
        config, current_month, drift_results
    )
    ...
    if overall_success:
        if drift_detected:
            logger.warning(
                f"⚠️ Monitoring completed with drift detected (Level: {alert_level})"
            )
        else:
    ...
    return {
        ...
        "data_quality": {"all_passed": quality_passed, "results": quality_results},
