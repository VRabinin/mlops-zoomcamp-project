...
def load_and_enhance_data_task(config: Config, snapshot_month: str) -> Tuple[bool, list[pd.DataFrame], list[str], Dict[str, Any]]:
...
def save_enhanced_data_task(dataframes: list[pd.DataFrame], filenames: list[str], config: Config, snapshot_month: str, suffix: str = "raw") -> Tuple[bool, list[str]]:
...
def crm_data_acquisition_flow(snapshot_month: str) -> Dict[str, Any]:
    ...
    load_success, dataframes, filenames, load_metadata = load_and_enhance_data_task(config, snapshot_month)
    
    if not load_success:
        logger.error(f"Pipeline failed at enhance step: {load_metadata.get('error', 'Unknown error')}")
        return {"status": "failed", "step": "load", "error": load_metadata.get('error')}
    
    # Task 3: Save cleaned data
    save_success, save_paths = save_enhanced_data_task(dataframes, filenames, config, snapshot_month,"raw")
