    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
                ...
                "deployments": [
                    {
                        ...
                        "parameters": {},
                        "work_pool": {"name": config.prefect.work_pool},
            ...
            for config_file in Path(temp_dir).rglob("*.yaml"):
                ...
                logger.info(f"📤 Uploading {relative_path} to MinIO S3...")
                s3_client.upload_file(str(config_file), data_lake_bucket, s3_key)
...
def main():
    ...
    try:
        ...
        print(
            f"2. Run flow manually: prefect deployment run crm_data_ingestion_flow/{deployment_name}"
        )
