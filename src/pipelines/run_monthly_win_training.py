...
def validate_training_data_task(df: pd.DataFrame) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        required_columns = ["is_won", "is_closed", "sales_cycle_days", "close_value"]
        ...
        if len(df) < 1000:
            validation_results["warnings"].append(
                f"Small dataset size: {len(df)} samples"
            )
        ...
        if "engage_date" in df.columns:
            try:
                ...
                df_temp = df.copy()
                df_temp["engage_date"] = pd.to_datetime(
                    df_temp["engage_date"], errors="coerce"
                )
                ...
                if len(valid_dates) > 0:
                    ...
                    validation_results["warnings"].append(
                        "No valid engage_date values found"
                    )
            except Exception as e:
                validation_results["warnings"].append(
                    f"Could not analyze engage_date: {str(e)}"
                )
        ...
        validation_results["win_rate"] = (
            df["is_won"].mean() if "is_won" in df.columns else None
        )
...
def train_monthly_win_models_task(
    ...
    try:
        ...
        if results["status"] == "success":
            ...
            logger.info(
                f"📝 Registered model: {results['model_info']['registered_model_name']}"
            )
            return True, results
        else:
            logger.error(
...
def save_training_results_task(
    results: Dict[str, Any], config: Config, snapshot_month: str
) -> Tuple[bool, str]:
...
def create_model_monitoring_setup_task(
    ...
    try:
        ...
        monitoring_setup = {
            ...
            "monitoring_metrics": {
                "performance_metrics": {
                    "roc_auc_threshold": max(
                        0.75, best_model.get("roc_auc", 0.8) * 0.95
                    ),
                    "brier_score_threshold": min(
                        0.25, best_model.get("brier_score", 0.2) * 1.1
                    ),
                    "accuracy_threshold": max(
                        0.7, best_model.get("accuracy", 0.75) * 0.95
                    ),
                },
                "data_drift_monitoring": {
                    "feature_count": results.get("dataset_info", {}).get("features", 0),
                    "win_rate_baseline": results.get("dataset_info", {}).get(
                        "win_rate", 0.3
                    ),
        ...
        logger.info(
            f"📊 ROC AUC threshold: {monitoring_setup['monitoring_metrics']['performance_metrics']['roc_auc_threshold']:.3f}"
        )
        logger.info(
            f"🎯 Brier score threshold: {monitoring_setup['monitoring_metrics']['performance_metrics']['brier_score_threshold']:.3f}"
        )
...
def monthly_win_probability_training_flow(snapshot_month: str):
    ...
    if not load_success:
        logger.error(
            f"Pipeline failed at load step: {load_metadata.get('error', 'Unknown error')}"
        )
        return {
            "status": "failed",
            "step": "load",
    ...
    if not training_success:
        logger.error(
            f"Pipeline failed at training step: {training_results.get('error', 'Unknown error')}"
        )
        return {
            "status": "failed",
            "step": "training",
    ...
    monitoring_success, monitoring_setup = create_model_monitoring_setup_task(
        training_results
    )

    if not monitoring_success:
        logger.warning(
            f"Failed to create monitoring setup: {monitoring_setup.get('error', 'Unknown error')}"
        )
        # Don't fail the pipeline for this
    ...
    logger.info(
        f"🏆 Best model: {summary['training']['best_model'].get('name', 'Unknown')}"
    )
    logger.info(f"📊 ROC AUC: {summary['training']['best_model'].get('roc_auc', 0):.4f}")
    logger.info(
        f"📝 Model registered: {summary['training']['model_info'].get('registered_model_name', 'Unknown')}"
    )
...
if __name__ == "__main__":
    ...
    if result["status"] == "success":
        ...
        print(
            f"📊 Data: {result['data_info']['samples']:,} samples, {result['data_info']['features']} features"
        )
        print(
            f"🏆 Best Model: {result['training']['best_model'].get('name', 'Unknown')}"
        )
        print(f"🎯 ROC AUC: {result['training']['best_model'].get('roc_auc', 0):.4f}")
        print(
            f"📝 Model: {result['training']['model_info'].get('registered_model_name', 'Unknown')}"
        )
