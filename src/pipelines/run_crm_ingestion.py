...
def load_and_clean_data_task(
    config: Config, snapshot_month: str
) -> Tuple[
    bool, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, Dict[str, Any]
]:
    ...
    try:
        ...
        # Load sales data
        df_sales = ingestion.load_data(
            Path(f"sales_pipeline_enhanced_{snapshot_month}.csv")
        )
...
def crm_data_ingestion_flow(snapshot_month: str):
    ...
    (
        ...
        df_sales_teams,
        load_metadata,
    ) = load_and_clean_data_task(config, snapshot_month)
    ...
    save_success, save_path = save_processed_data_task(
        df_cleaned, config, "processed", snapshot_month
    )
    ...
    validation_passed, validation_results = validate_data_quality_task(
        df_cleaned, config
    )

    # Task 4: Engineer features
    features_success, df_features, features_metadata = engineer_features_task(
        df_cleaned, df_accounts, df_products, df_sales_teams, config
    )

    if not features_success:
        logger.error(
            f"Pipeline failed at feature engineering: {features_metadata.get('error', 'Unknown error')}"
        )
        return {
            "status": "failed",
            "step": "features",
            "error": features_metadata.get("error"),
        }

    # Task 5: Save features
    features_save_success, features_path = save_processed_data_task(
        df_features, config, "features", snapshot_month
    )
    ...
    summary = {
        ...
        "files": {"processed_data": save_path, "features": features_path},
    }
