...
def create_reference_data_task(config: Config, snapshot_month: str, sample_size: int) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        success, message = drift_monitor.create_reference_data(snapshot_month, sample_size)
...
def validate_reference_data_task(config: Config, snapshot_month: str) -> Tuple[bool, Dict[str, Any]]:
    """
    ...
    try:
        ...
        if "prediction" in df.columns:
            validation_results["prediction_range"] = [float(df["prediction"].min()), float(df["prediction"].max())]
            validation_results["missing_predictions"] = int(df["prediction"].isna().sum())
        
        # Check if validation passes
        validation_results["valid"] = (
            validation_results["has_predictions"] and
            validation_results["has_target"] and
            validation_results["missing_predictions"] == 0 and
            df.shape[0] > 0
        )
...
def create_reference_data_flow(
    snapshot_month: str,
    sample_size: int
) -> Dict[str, Any]:
    """
