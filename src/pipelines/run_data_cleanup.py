...
def identify_cleanup_files_task(
    ...
    try:
        ...
        if hasattr(config.data_path, "temp_data") or hasattr(
        ...
        logger.info(
            f"  📊 Processed files: {len(cleanup_categories['processed_files'])}"
        )
        logger.info(f"  🔧 Feature files: {len(cleanup_categories['feature_files'])}")
        logger.info(
            f"  📈 Monitoring files: {len(cleanup_categories['monitoring_files'])}"
        )
        logger.info(
            f"  🎯 Prediction files: {len(cleanup_categories['prediction_files'])}"
        )
...
def cleanup_summary_task(
    cleanup_results: List[Tuple[str, bool, str]]
) -> Dict[str, Any]:
    ...
    for category, success, message in cleanup_results:
        summary["details"].append(
            {"category": category, "success": success, "message": message}
        )
    ...
    try:
        ...
        if include_processed and file_categories.get("processed_files"):
            cleanup_tasks.append(
                ("processed_files", file_categories["processed_files"])
            )
        ...
        if include_monitoring and file_categories.get("monitoring_files"):
            cleanup_tasks.append(
                ("monitoring_files", file_categories["monitoring_files"])
            )

        if include_predictions and file_categories.get("prediction_files"):
            cleanup_tasks.append(
                ("prediction_files", file_categories["prediction_files"])
            )
        ...
        for category_name, file_list in cleanup_tasks:
            success, message = cleanup_file_category_task(
                config_dict, category_name, file_list, dry_run
            )
