...
class MonthlyWinProbabilityTrainer:
    ...
    def create_monthly_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.logger.info(
            f"Simulating predictions for: {current_date.strftime('%Y-%m-%d')}"
        )

        # Calculate days since engagement
        df_monthly["days_since_engage"] = (
            current_date - df_monthly["engage_date"]
        ).dt.days
        ...
        df_monthly["expected_close_date"] = df_monthly["engage_date"] + timedelta(
            days=avg_sales_cycle
        )
        df_monthly["days_to_expected_close"] = (
            df_monthly["expected_close_date"] - current_date
        ).dt.days

        # Monthly prediction flags
        df_monthly["should_close_next_month"] = df_monthly[
        ...
        valid_sales_cycle_mask = (df_monthly["sales_cycle_days"].notna()) & (
            df_monthly["sales_cycle_days"] > 0
        )
        ...
        df_monthly["high_value_deal"] = df_monthly["close_value"] > df_monthly[
            "close_value"
        ].quantile(0.8)
        df_monthly["long_sales_cycle"] = (
            df_monthly["sales_cycle_days"]
            > df_monthly["sales_cycle_days"].quantile(0.8)
        ).fillna(False)
        ...
        self.logger.info(
            f"- Opportunities that should close next month: {df_monthly['should_close_next_month'].sum():,}"
        )
        self.logger.info(f"- Overdue opportunities: {df_monthly['is_overdue'].sum():,}")
        self.logger.info(
            f"- Early stage opportunities: {df_monthly['is_early_stage'].sum():,}"
        )

        return df_monthly

    def prepare_monthly_prediction_data(
        ...
        training_data = df[
            (df["is_closed"] == 1)
            | (  # Include all closed deals for training
                df["should_close_next_month"] == True
            )  # Include current pipeline
        ].copy()
        ...
        self.logger.info(
            f"Open deals for prediction: {(training_data['is_closed'] == 0).sum():,}"
        )
        ...
        missing_features = [
            f for f in monthly_features if f not in training_data.columns
        ]
        ...
        self.logger.info(
            f"Using {len(available_features)} features for monthly prediction"
        )
    ...
    def create_temporal_split(
        ...
        if len(closed_indices) > 0:
            ...
            self.logger.info(
                f"Found {len(closed_indices):,} closed deals for temporal split"
            )
            ...
            if "close_date" in closed_data_with_index.columns:
                ...
                closed_data_with_index.loc[
                    missing_close_date, "close_date"
                ] = pd.to_datetime(
                    closed_data_with_index.loc[missing_close_date, "engage_date"]
                ) + timedelta(
                    days=avg_cycle
                )

                # Sort by close_date
                closed_data_with_index["close_date"] = pd.to_datetime(
                    closed_data_with_index["close_date"]
                )
            ...
            self.logger.info(
                f"Temporal split: {len(sorted_indices)} closed deals, {split_idx} train, {len(sorted_indices) - split_idx} test"
            )
        ...
        self.logger.info(
            f"Training set: {len(X_train):,} samples, win rate: {y_train.mean():.3f}"
        )
        self.logger.info(
            f"Test set: {len(X_test):,} samples, win rate: {y_test.mean():.3f}"
        )
        self.logger.info(f"Features: {X_train.shape[1]}")

        return X_train, X_test, y_train, y_test

    def train_monthly_win_models(
        self,
    ...
    def generate_training_summary(
        ...
        summary = {
            ...
            "model_performance_ranking": results_df.sort_values(
    ...
    def run_training_pipeline(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            X, y, feature_list, training_data = self.prepare_monthly_prediction_data(
                df_monthly
            )
            ...
            self.logger.info(
                f"✨ Model registered: {model_info_dict['registered_model_name']} v{model_info_dict['model_version']}"
            )
