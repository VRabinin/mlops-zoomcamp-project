...
class Config:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "Config":
        ...
        data_path_dict = config_dict.get("data_path", {})
        mlflow_dict = config_dict.get("mlflow", {})
        prefect_dict = config_dict.get("prefect", {})
        monitoring_dict = config_dict.get("monitoring", {})
        storage_dict = config_dict.get("storage", {})

        return cls(
            first_snapshot_month=config_dict.get("first_snapshot_month", "XXXX-XX"),
            data_path=DataPathConfig(**data_path_dict)
            if data_path_dict
            else DataPathConfig(),
            mlflow=MLflowConfig(**mlflow_dict) if mlflow_dict else MLflowConfig(),
            prefect=PrefectConfig(**prefect_dict) if prefect_dict else PrefectConfig(),
            monitoring=MonitoringConfig(**monitoring_dict)
            if monitoring_dict
            else MonitoringConfig(),
            storage=StorageConfig(**storage_dict) if storage_dict else StorageConfig(),
            environment=config_dict.get("environment", "development"),
            debug=config_dict.get("debug", True),
            log_level=config_dict.get("log_level", "INFO"),
            database_url=config_dict.get(
                "database_url",
...
def get_config(config_path: Optional[str] = None) -> Config:
    ...
    config.data_path.processed = os.getenv(
        "PROCESSED_DATA_PATH", config.data_path.processed
    )
    config.data_path.features = os.getenv(
        "FEATURE_STORE_PATH", config.data_path.features
    )
    ...
    config.mlflow.artifact_location = os.getenv(
        "MLFLOW_ARTIFACT_LOCATION", config.mlflow.artifact_location
    )
    ...
    config.storage.endpoint_url = os.getenv(
    ...
    config.storage.secret_key = os.getenv(
        "MINIO_ROOT_PASSWORD", config.storage.secret_key
    )
    config.storage.region = os.getenv("AWS_DEFAULT_REGION", config.storage.region)

    # Override bucket names from environment
    config.storage.buckets["mlflow_artifacts"] = os.getenv(
        "MLFLOW_ARTIFACTS_BUCKET", config.storage.buckets["mlflow_artifacts"]
    )
    config.storage.buckets["data_lake"] = os.getenv(
        "DATA_LAKE_BUCKET", config.storage.buckets["data_lake"]
    )
    config.storage.buckets["model_artifacts"] = os.getenv(
        "MODEL_ARTIFACTS_BUCKET", config.storage.buckets["model_artifacts"]
    )
    ...
    s3_processed_path = os.getenv(
        "S3_PROCESSED_DATA_PATH", config.storage.s3_paths["processed"]
    )
    s3_features_path = os.getenv(
        "S3_FEATURES_DATA_PATH", config.storage.s3_paths["features"]
    )
    s3_models_path = os.getenv("S3_MODELS_PATH", config.storage.s3_paths["models"])
    s3_experiments_path = os.getenv(
        "S3_EXPERIMENTS_PATH", config.storage.s3_paths["experiments"]
    )
    s3_prefect_path = os.getenv(
        "S3_PREFECT_FLOWS_PATH", config.storage.s3_paths["prefect_flows"]
    )
