...
class StorageManager:
    ...
    def __init__(self, config: Config):
        ...
        self.logger.info(
            f"Storage mode: {'S3/MinIO' if self.use_s3 else 'Local filesystem'}"
        )
    ...
    def _should_use_s3(self) -> bool:
        ...
        if os.getenv("PREFECT_WORKER_TYPE") or os.getenv(
            "PREFECT_API_URL", ""
        ).startswith("https://"):
    ...
    def _setup_s3_client(self):
        ...
        try:
            ...
            self.s3_client = boto3.client(
                ...
                if hasattr(self.config.storage, "endpoint_url")
                else None,
    ...
    def _save_dataframe_s3(
        self, df: pd.DataFrame, bucket: str, path: str, **kwargs
    ) -> str:
    ...
    def list_files(self, prefix: str = "") -> list:
        ...
        if self.use_s3:
            return self._list_files_s3(
                self.config.storage.buckets.get("data_lake"), prefix
            )
        else:
    ...
    def get_s3_path(self, data_type: str, filename: str) -> str:
        ...
        if data_type not in self.config.storage.s3_paths:
            raise ValueError(
                f"Unknown data type: {data_type}. Available: {list(self.config.storage.s3_paths.keys())}"
            )
    ...
    def save_dataframe(
        self, df: pd.DataFrame, data_type: str, filename: str, **kwargs
    ) -> str:
    ...
    def load_dataframe_by_type(
        self, data_type: str, filename: str, **kwargs
    ) -> pd.DataFrame:
    ...
    def resolve_path(self, data_type: str, filename: str = None) -> Union[str, Path]:
        ...
        if self.use_s3:
            ...
            if data_type not in self.config.data_path.__dict__:
                raise ValueError(
                    f"Unknown data type: {data_type}. Available: {list(self.config.data_path.__dict__.keys())}"
                )
    ...
    def upload_file(
        self, file_obj: Union[BinaryIO, io.StringIO], data_type: str, filename: str
    ):
    ...
    def _upload_file_s3(
        self, file_obj: Union[BinaryIO, io.StringIO], bucket: str, key: str
    ):
        """Upload file to S3."""
        try:
            self._ensure_bucket_exists(bucket)
            ...
            if isinstance(file_obj, io.StringIO):
                self.s3_client.put_object(
                    Bucket=bucket, Key=key, Body=file_obj.getvalue().encode("utf-8")
                )
            else:
                file_obj.seek(0)
                self.s3_client.put_object(Bucket=bucket, Key=key, Body=file_obj.read())
        except Exception as e:
